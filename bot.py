...
def get_all_subscribers_list() -> str:
    try:
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT user_id, username, first_name, joined_at "
                    "FROM subscribers ORDER BY joined_at ASC"
                )
                rows = cur.fetchall()
        if not rows:
        ...
        for i, sub in enumerate(rows, start=1):
            ...
            lines.append(
                f"{i}. user_id: {sub['user_id']}, username: {username}, "
                f"name: {name}, joined: {joined}"
            )
        return "\n".join(lines)
    except Exception as e:
...
# ---------------------------------------------------------------------------
# Helper: draft management
# ---------------------------------------------------------------------------

def _get_draft(context: ContextTypes.DEFAULT_TYPE) -> dict:
    """Returns the current draft from user_data, creating it if absent."""
...
def _clear_draft(context: ContextTypes.DEFAULT_TYPE):
    """Clears the current draft and all related state."""
...
def _parse_buttons_from_text(text: str) -> tuple[str, list]:
    """Parses inline button definitions from text.
    ...
    for line in lines:
        ...
        if m:
            btn_text, url = m.group(1).strip(), m.group(2).strip()
            if url.startswith("http") or "t.me" in url:
        ...
        # Format: Text|url  (plain pipe-separated)
        if "|" in stripped:
...
def _draft_status_text(draft: dict) -> str:
    """Returns a human-readable summary of the current draft state."""
    parts = []
    if draft.get("text"):
    ...
    parts.append(f"💾 ID в БД: #{db_id}" if db_id else "💾 Не сохранён")
    return "\n".join(parts)


def _save_draft_to_db(admin_id: int, draft: dict) -> int:
    """Saves (insert or update) the draft in the DB and returns its id."""
...
def _build_reply_markup(buttons: list) -> InlineKeyboardMarkup | None:
    """Builds an InlineKeyboardMarkup from a list of {text, url} dicts."""
    if not buttons:
        return None
...
def _draft_keyboard() -> InlineKeyboardMarkup:
    """Returns the standard draft action keyboard."""
...
async def _send_draft_preview(target, context: ContextTypes.DEFAULT_TYPE, draft: dict):
    """Sends a preview of the draft.

    `target` must be a Message object (supports reply_text / reply_photo).
    """
    reply_markup_content = _build_reply_markup(draft.get("buttons", []))
...
async def _do_send_broadcast(
    ...
    """Executes the broadcast and updates the DB record.

    `reply_target` must be a Message object.
    """
    ...
    reply_markup = _build_reply_markup(draft.get("buttons", []))
    text = draft.get("text") or ""
    sent = 0
    failed = 0
...
def _main_menu_keyboard() -> InlineKeyboardMarkup:
    """Returns the main broadcast management menu keyboard."""
    return InlineKeyboardMarkup([
        [
...
def _get_broadcasts_page(page: int, per_page: int = 5) -> tuple[list, int]:
    """Returns one page of broadcasts and the total count."""
    try:
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
...
def _get_broadcast_by_id(broadcast_id: int) -> dict | None:
    """Returns a broadcast record by its ID."""
    try:
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
...
def _delete_broadcast(broadcast_id: int) -> bool:
    """Deletes a draft broadcast. Returns True on success."""
    try:
        with psycopg.connect(DATABASE_URL) as conn:
            with conn.cursor() as cur:
                cur.execute(
...
def _get_subscribers_stats() -> dict:
    ...
    try:
        ...
        return {"total": total, "recent": recent, "week": week_count, "month": month_count}
    except Exception as e:
...
def _history_keyboard(
    rows: list, page: int, total: int, per_page: int = 5
) -> InlineKeyboardMarkup:
    """Builds the keyboard for the broadcast history page."""
    buttons = []
    for r in rows:
...
def _broadcast_detail_keyboard(broadcast_id: int, status: str) -> InlineKeyboardMarkup:
    """Returns the action keyboard for a broadcast detail view."""
    rows = [
...
def _delete_confirm_keyboard(broadcast_id: int) -> InlineKeyboardMarkup:
    """Returns the delete-confirmation keyboard."""
    return InlineKeyboardMarkup([
        [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Меня зовут <b>Арлан Алтынов</b> — профессиональный трейдер с более чем 8-летним опытом "
        f"торговли на Forex и CFD.\n"
        ...
        f"🔥 <b>Для новичков</b> — провожу полноценное обучение с нуля: от основ трейдинга до "
        f"уверенной торговли по моим сигналам.\n\n"
        ...
        f"Я лично помогаю своим подписчикам выходить на стабильную прибыль. "
        f"Многие уже успешно торгуют благодаря моим сигналам и поддержке.\n\n"
...
async def draft_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📝 Новый черновик создан. Отправляй текст и фото — они добавятся автоматически.",
        reply_markup=_draft_keyboard(),
    )
...
async def menu_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Broadcast history (paginated) ────────────────────────────────────────
        ...
        total_pages = (total + per_page - 1) // per_page
        text = f"📊 <b>История рассылок</b> (стр. {page + 1}/{total_pages}, всего: {total}):\n\n"
        for r in rows:
            status_icon = "✅" if r["status"] == "sent" else "📝"
            created = r["created_at"].strftime("%d.%m.%Y %H:%M") if r["created_at"] else "—"
            preview = (r["preview"] or "").replace("\n", " ")
    ...
    # ── Confirmed delete ─────────────────────────────────────────────────────
        ...
        if success:
            current = context.user_data.get("current_draft")
            if current and current.get("db_id") == broadcast_id:
                _clear_draft(context)
            await query.message.reply_text(
...
async def draft_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Back to menu (with unsaved-changes check) ────────────────────────────
        ...
        current = context.user_data.get("current_draft")
        has_content = current and (current.get("text") or current.get("photo_file_id"))
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Accepts text from an admin while a draft is active and appends it."""
    if not is_admin(update.effective_user.id):
        return
    if "current_draft" not in context.user_data:
        return
    ...
    btn_note = f" (+{len(new_buttons)} кнопок)" if new_buttons else ""
    await update.message.reply_text(
        f"✅ Текст добавлен ({char_count} символов){btn_note}",
        reply_markup=_draft_keyboard(),
    )


async def admin_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Accepts a photo from an admin while a draft is active and attaches it."""
    if not is_admin(update.effective_user.id):
        return
    if "current_draft" not in context.user_data:
        return
...
def main():
    ...
    # Admin message handlers (only when a draft is active)
    app.add_handler(
        MessageHandler(
