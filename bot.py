    ...
    try:
        with psycopg.connect(DATABASE_URL) as conn:
            with conn.cursor() as cur:
                ...
                """)
                cur.execute("""
                    CREATE INDEX IF NOT EXISTS idx_subscribers_joined
...
def add_subscriber(user_id: int, username: str | None, first_name: str | None):
    try:
        with psycopg.connect(DATABASE_URL) as conn:
            with conn.cursor() as cur:
                cur.execute("""
    ...
    try:
        with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT user_id, username, first_name, joined_at "
                    "FROM subscribers ORDER BY joined_at DESC LIMIT 10"
                )
                recent = cur.fetchall()
...
def _parse_buttons_from_text(text: str) -> tuple[str, list]:
    ...
    for line in lines:
        ...
        if m:
            ...
            if btn_url.startswith("http") or "t.me" in btn_url:
...
def _draft_status_text(draft: dict) -> str:
    """Returns a human-readable status string for the current draft."""
    parts = []
    if draft.get("text"):
...
def _draft_keyboard() -> InlineKeyboardMarkup:
    """Keyboard shown while editing a draft."""
...
async def _send_draft_preview(target, context: ContextTypes.DEFAULT_TYPE, draft: dict):
    ...
    `target` must expose reply_text / reply_photo (a Message object).
    """
...
async def _do_send_broadcast(
    ...
    """Executes the broadcast and updates the DB. `reply_target` is a Message."""
...
async def users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(subscribers_list) > 4000:
        subscribers_list = subscribers_list[:4000] + "\n…(обрезано)"
    ...
    # ── History (paginated) ──────────────────────────────────────────────────
        ...
        for r in rows:
            ...
            if r["status"] == "sent" and r["sent_at"]:
                sent_info = (
                    f" | 📤 {r['sent_at'].strftime('%d.%m %H:%M')} ({r['sent_count']} польз.)"
                )
            text += (
                f"{status_icon} <b>#{r['id']}</b> [{r['status']}] {created}{sent_info}\n"
                f"   {preview}\n\n"
                ...
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton(
                            "💾 Сохранить и выйти", callback_data="draft_save_and_exit"
                        ),
                        InlineKeyboardButton(
                            "🗑 Выйти без сохранения", callback_data="draft_discard_and_exit"
                        ),
                    ],
                ]),
            )
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    btn_info = f" (+{len(new_buttons)} кнопок)" if new_buttons else ""
    await update.message.reply_text(
        f"✅ Текст добавлен ({char_count} символов){btn_info}\n\n"
...
def main():
    ...
    # Admin message input (photo first so it takes priority over text)
    app.add_handler(
        MessageHandler(
